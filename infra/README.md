...
`@handshake/shared` (schemas/types) and `@handshake/resolver` (`resolve`,
...
| Name              | Where              | Purpose                                             |
| ----------------- | ------------------ | --------------------------------------------------- |
| `HandshakeToken`  | deploy parameter   | Static workspace token; becomes each Lambda's `HANDSHAKE_TOKEN`. Callers send `Authorization: Bearer <token>`. |
| `HANDSHAKE_TOKEN` | Lambda env (auto)  | Set from the parameter by the template.             |
| `TABLE_NAME`      | Lambda env (auto)  | Set to the `handshake` table by the template.       |
...
# first time — interactive; pick region ap-south-1 and enter HandshakeToken
sam deploy --guided

# or non-interactive:
sam deploy `
  --stack-name handshake `
  --region ap-south-1 `
  --capabilities CAPABILITY_IAM `
  --resolve-s3 `
  --no-confirm-changeset `
  --parameter-overrides "HandshakeToken=<your-token>"
```
